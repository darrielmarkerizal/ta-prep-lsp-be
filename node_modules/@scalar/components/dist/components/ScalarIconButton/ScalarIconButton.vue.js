import { defineComponent as d, useTemplateRef as c, computed as o, createElementBlock as s, openBlock as a, mergeProps as u, unref as i, createVNode as p, createCommentVNode as m, toDisplayString as f } from "vue";
import { cva as b, useBindCx as h } from "@scalar/use-hooks/useBindCx";
import { BUTTON_VARIANT_STYLES as g } from "../ScalarButton/constants.js";
import { useTooltip as v } from "../ScalarTooltip/useTooltip.js";
import z from "../ScalarIcon/ScalarIconLegacyAdapter.vue.js";
const y = ["aria-disabled"], k = {
  key: 0,
  class: "sr-only"
}, B = {}, C = /* @__PURE__ */ d({
  ...B,
  inheritAttrs: !1,
  __name: "ScalarIconButton",
  props: {
    label: {},
    icon: { type: [Object, Function] },
    disabled: { type: Boolean },
    variant: { default: "ghost" },
    size: { default: "md" },
    thickness: {},
    weight: {},
    tooltip: { type: [Boolean, Object] }
  },
  setup(t) {
    const n = b({
      base: "scalar-icon-button grid aspect-square cursor-pointer rounded",
      variants: {
        size: {
          xxs: "size-3.5 p-0.5",
          xs: "size-5 p-1",
          sm: "size-6 p-1",
          md: "size-10 p-3",
          full: "size-full"
        },
        disabled: {
          true: "cursor-not-allowed shadow-none"
        },
        // We use the same button styles for the icon buttons
        variant: g
      },
      compoundVariants: [
        {
          size: "md",
          variant: "ghost",
          class: "size-8 p-2 m-1 -outline-offset-2 rounded-lg"
        }
      ]
    }), { cx: r } = h(), l = c("ref");
    return v({
      content: o(() => t.label),
      offset: o(() => t.variant === "ghost" ? 0 : 4),
      placement: o(
        () => typeof t.tooltip == "boolean" ? void 0 : t.tooltip
      ),
      targetRef: o(() => t.tooltip ? l.value : void 0)
    }), (e, w) => (a(), s("button", u({
      ref: "ref",
      "aria-disabled": e.disabled,
      type: "button"
    }, i(r)(i(n)({ size: e.size, variant: e.variant, disabled: e.disabled }))), [
      p(i(z), {
        icon: e.icon,
        thickness: e.thickness,
        weight: e.weight
      }, null, 8, ["icon", "thickness", "weight"]),
      e.tooltip ? m("", !0) : (a(), s("span", k, f(e.label), 1))
    ], 16, y));
  }
});
export {
  C as default
};
