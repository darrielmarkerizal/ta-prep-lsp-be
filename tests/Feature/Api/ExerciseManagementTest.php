...
describe('Exercise Management', function () {
    ...
    it('can list exercises with filters', function () {
        ...
        $response = $this->actingAs($this->admin, 'api')->getJson(api('/assessments/exercises') . '?scope_type=course&scope_id=' . $this->course->id);

        $response->assertStatus(200)
            ->assertJsonPath('data.data.0.id', $exercise->id);
    });

    it('can view exercise details', function () {
        $exercise = Exercise::factory()->create(['created_by' => $this->instructor->id]);

        $response = $this->actingAs($this->instructor, 'api')->getJson(api("/assessments/exercises/{$exercise->id}"));
    ...
    it('can update draft exercise', function () {
        $exercise = Exercise::factory()->create(['created_by' => $this->instructor->id, 'status' => 'draft']);

        $response = $this->actingAs($this->instructor, 'api')->putJson(api("/assessments/exercises/{$exercise->id}"), [
            'title' => 'Updated Quiz',
            'max_score' => 150,
        ]);

        $response->assertStatus(200)
            ->assertJsonPath('data.exercise.title', 'Updated Quiz');
    });

    it('cannot update published exercise', function () {
        $exercise = Exercise::factory()->create(['created_by' => $this->instructor->id, 'status' => 'published']);
    ...
    it('can delete draft exercise', function () {
        ...
        $response = $this->actingAs($this->instructor, 'api')->deleteJson(api("/assessments/exercises/{$exercise->id}"));
    ...
    it('can publish exercise with questions', function () {
        ...
        $response = $this->actingAs($this->instructor, 'api')->putJson(api("/assessments/exercises/{$exercise->id}/publish"));

        $response->assertStatus(200)
            ->assertJsonPath('data.exercise.status', 'published');
    });
    ...
    it('can get exercise questions', function () {
        ...
        $response = $this->actingAs($this->instructor, 'api')->getJson(api("/assessments/exercises/{$exercise->id}/questions"));
